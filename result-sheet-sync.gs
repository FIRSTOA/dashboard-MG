...
function readGoals_(gid, goalColParam, noMonths, quarter) {
  ...
  // 빈 구역에서 첫 목표를 넣을 기준 행(소제목 행). 이 행 다음(=첫 목표 칸)에 삽입됨. 구역 없으면 시트 끝.
  var anchor = win ? (START_ROW + win.start + 1) : last;
  ...
  for (var i = lo; i < hi; i++) {
    ...
    var months = monthCols.map(function (mc) {
      return { col: mc.col, label: mc.label, text: String(values[i][mc.col - 1] || ''), html: cellHtml_(rich[i][mc.col - 1]) }; // 월칸도 색상 유지
    });
    rows.push({
      row: START_ROW + i,
